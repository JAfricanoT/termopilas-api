...
import type { AllDevicesRoute, CreateDeviceRoute, GetDeviceRoute, PatchDeviceRoute } from "./devices.routes";

export const allDevices: AppRouteHandler<AllDevicesRoute> = async (c) => {
  const allDevices = await postgres
    .select()
    .from(devices);
...
export const createDevice: AppRouteHandler<CreateDeviceRoute> = async (c) => {
  ...
  const [insertedDevice] = await postgres
    .insert(devices)
    .values(device)
    .returning();
